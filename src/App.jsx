...
function App() {
  ...
  const [dockPosition, setDockPosition] = useState(() => localStorage.getItem('brainshake-dock') || 'bottom')
  ...
  useEffect(() => { localStorage.setItem('brainshake-boards-v1', JSON.stringify(boards)) }, [boards])
  ...
  useEffect(() => { localStorage.setItem('brainshake-dock', dockPosition) }, [dockPosition])
  ...
  function dataUrlInfo(src, id) { const match = /^data:([^;,]+)?((?:;[^,]*)?),([ -]*)$/s.exec(src); if (!match) return null; const mediaType = match[1] || 'application/octet-stream'; const metadata = match[2] || ''; const extension = ({ 'image/jpeg': 'jpg', 'image/svg+xml': 'svg', 'image/png': 'png', 'image/gif': 'gif', 'image/webp': 'webp', 'video/mp4': 'mp4', 'video/webm': 'webm', 'text/html': 'html' })[mediaType] || 'bin'; return { mediaType, extension, data: match[3], base64: metadata.includes(';base64'), id: id.replace(/[^a-z0-9_-]/gi, '-') } }
  async function exportBrainshake() { const zip = new JSZip(); const media = []; const objects = board.objects.map(item => { if (!['image', 'video', 'html'].includes(item.type) || !item.src?.startsWith('data:')) return { ...item }; const info = dataUrlInfo(item.src, item.id); if (!info) return { ...item }; const path = `media/${info.id}.${info.extension}`; const content = info.base64 ? info.data : decodeURIComponent(info.data); media.push({ path, content, base64: info.base64 }); return { ...item, src: path, mediaType: info.mediaType } }); zip.file('board.json', JSON.stringify({ name: board.name, objects }, null, 2)); media.forEach(item => zip.file(item.path, item.content, { base64: item.base64 })); downloadBlob(await zip.generateAsync({ type: 'blob' }), boardFilename('brainshake')); setToast('Board exported') }
  ...
  function beginPan(event) { if (tool !== 'hand' || event.button !== 0) return; event.currentTarget.setPointerCapture?.(event.pointerId); event.preventDefault(); setDragging({ type: 'pan', start: { x: event.clientX, y: event.clientY }, origin: pan }) }
  ...
  function finishStroke(stroke) {
    ...
    const bounds = stroke.points.reduce((result, point) => ({
  ...
  function endPointer(event) {
    if (drawing) { updateBoard(current => ({ ...current, objects: [...current.objects, finishStroke(drawing)] })); setDrawing(null) }
  ...
  function submitImageUrl(url) {
    if (!/^https?:\/\//i.test(url)) { setToast('Enter a valid image URL'); return }
  ...
  function onWheel(event) {
    ...
    const nextZoom = Math.min(2.4, Math.max(.35, zoom * Math.pow(0.9985, delta)))
    setZoom(nextZoom)
  ...
  return <div className={`app theme-${theme}`} style={{ '--accent': accent }} onClick={() => setContext(null)}>
    <header className="topbar">
      ...
      <div className="top-actions"><span className="save-state"><i className="save-dot" /> Saved locally</span><button className="icon-button" title="Search"><Search size={17} /></button><ExportMenu onExport={exportBrainshake} onExportJson={exportJson} compact /><button className="icon-button" title="Import board" onClick={() => boardFileRef.current?.click()}><Upload size={17} /></button><button className="icon-button" title="Settings" onClick={() => setShowPanel(value => !value)}><Settings2 size={17} /></button></div>
    </header>
    <aside className="sidebar">
      ...
      <div className="sidebar-section"><div className="section-label">Boards <button className="icon-button" style={{ display: 'inline-grid', width: 20, height: 20 }} title="New board" onClick={createBoard}><Plus size={14} /></button></div><div className="board-list">{boards.map(item => <button key={item.id} className={'board-item ' + (item.id === board.id ? 'active' : '')} onClick={() => switchBoard(item.id)}><span>{item.name}</span>{boards.length > 1 && <span className="board-delete" role="button" aria-label="Delete board" onClick={event => { event.stopPropagation(); deleteBoard(item.id) }}><X size={13} /></span>}</button>)}</div></div>
      ...
      <div className="sidebar-foot"><div className="sidebar-foot-copy">Everything stays in your browser.<br />No account. No cloud. Just ideas.</div><div className="sidebar-links"><a className="sidebar-link" href="https://github.com/pxdritz1/BrainShake" target="_blank" rel="noreferrer" title="Open GitHub repository"><GithubIcon size={16} /><span>Source</span></a><a className="sidebar-link" href="https://ko-fi.com/pxdritz1" target="_blank" rel="noreferrer" title="Support on Ko-fi"><CoffeeIcon size={16} /><span>Ko-fi</span></a></div></div>
    </aside>
    <main className="workspace">
      <div ref={canvasRef} className={`canvas-shell ${grid ? '' : 'grid-off'} ${dragging?.type === 'pan' ? 'is-panning' : ''}`} onWheel={onWheel} onPointerDown={handleCanvasPointerDown} onPointerMove={movePointer} onPointerUp={endPointer} onPointerCancel={endPointer} onDragOver={event => { event.preventDefault(); setDropActive(true) }} onDragLeave={() => setDropActive(false)} onDrop={event => { event.preventDefault(); setDropActive(false); importFiles(event.dataTransfer.files) }} onContextMenu={event => { event.preventDefault(); setContext({ x: event.clientX, y: event.clientY }) }}>
        <div className="canvas-world" style={{ transform: `translate(${pan.x}px, ${pan.y}px) scale(${zoom})`, transition: dragging?.type === 'pan' ? 'none' : 'transform 140ms cubic-bezier(.2,.75,.25,1)' }}>
      ...
      {dockDragging && <DockDropZones onDrop={position => { setDockPosition(position); setDockDragging(false) }} />}<Toolbar dockPosition={dockPosition} onDockDragStart={() => setDockDragging(true)} onDockDragEnd={() => setDockDragging(false)} tool={tool} setTool={value => value === 'image' || value === 'video' || value === 'html' ? fileRef.current?.click() : setTool(value)} strokeWidth={strokeWidth} setStrokeWidth={setStrokeWidth} undo={undo} redo={redo} canUndo={past.length > 0} canRedo={future.length > 0} addObject={addObject} />
      ...
      {showPanel && <Properties dockPosition={dockPosition} setDockPosition={setDockPosition} item={selectedItem} accent={accent} setAccent={setAccent} theme={theme} setTheme={setTheme} grid={grid} setGrid={setGrid} onClose={() => setShowPanel(false)} onChange={(id, patch) => updateObject(id, patch)} />}
...
function CanvasObject({ item, selected, onSelect, onDrag, onResize, onChange, onRemove }) {
  ...
  const markdownEditor = (className, placeholder) => item.editing
    ? <textarea className={className} value={item.text} placeholder={placeholder} autoFocus onBlur={() => onChange(item.id, { editing: false }, false)} onChange={event => onChange(item.id, { text: event.target.value })} />
    : <button type="button" className="markdown-preview" onClick={() => onChange(item.id, { editing: true }, false)}>{markdownContent}</button>
  if (item.type === 'sticky') content = <div className={`widget-body sticky ${item.color || 'yellow'}`} style={{ background: colors[item.color] }}><h3>{item.title || 'Note'}</h3>{markdownEditor('sticky-text', 'Write a note in Markdown...')}</div>
  if (item.type === 'text') content = <div className="widget-body text-card">{markdownEditor('', 'Write your idea in Markdown...')}</div>
  ...
  if (item.type === 'stroke') return <div {...common} className={`canvas-object stroke-object ${selected ? 'selected' : ''}`} onPointerDown={event => { common.onPointerDown(event); onDrag(event, item) }}><svg className="stroke" viewBox={`0 0 ${item.w} ${item.h}`}><path style={{ strokeWidth: item.strokeWidth || 4 }} d={item.points.map((point, index) => `${index ? 'L' : 'M'} ${point.x} ${point.y}`).join(' ')} /></svg>{resize}</div>
...
function LegacyToolbar({ tool, setTool, strokeWidth, setStrokeWidth, undo, redo, canUndo, canRedo }) {
...
function DockDropZones({ onDrop }) {
  const drop = position => event => { event.preventDefault(); if (event.dataTransfer.getData('text/plain') === 'toolbar-dock') onDrop(position) }
  return <div className="dock-drop-zones"><div className="dock-drop-zone dock-drop-top" onDragOver={event => event.preventDefault()} onDrop={drop('top')}>Top</div><div className="dock-drop-zone dock-drop-right" onDragOver={event => event.preventDefault()} onDrop={drop('right')}>Right</div><div className="dock-drop-zone dock-drop-bottom" onDragOver={event => event.preventDefault()} onDrop={drop('bottom')}>Bottom</div><div className="dock-drop-zone dock-drop-left" onDragOver={event => event.preventDefault()} onDrop={drop('left')}>Left</div></div>
}
...
function ExportMenu({ onExport, onExportJson, compact = false }) {
  ...
  return <div ref={menuRef} className={`export-menu ${compact ? 'compact' : ''}`} onClick={event => event.stopPropagation()}><button className={compact ? 'icon-button' : 'nav-item'} title="Export board" onClick={() => setOpen(value => !value)}>{compact ? <Download size={17} /> : <><Download size={16} /><span>Export</span></>}</button>{open && <div className="export-popover"><button onClick={() => choose(onExport)}><FileArchive size={16} /><span><strong>.brainshake</strong><small>Includes local media when supported</small></span></button><div className="export-note">External URLs and non-base64 media may not be bundled.</div><button onClick={() => choose(onExportJson)}><FileText size={16} /><span><strong>.json</strong><small>Compact, without media</small></span></button></div>}</div>
}

function LegacyProperties({ item, accent, setAccent, theme, setTheme, grid, setGrid, onClose, onChange }) {
  return <div className="floating-panel"><div className="panel-heading"><span><PanelRight size={14} /> Properties</span><button className="icon-button" title="Close properties" onClick={onClose}><X size={14} /></button></div>{item ? <><div className="panel-row"><span>Type</span><strong>{item.type}</strong></div><div className="panel-row"><span>Position</span><span>{Math.round(item.x)} × {Math.round(item.y)}</span></div><div className="panel-row"><span>Size</span><span>{Math.round(item.w)} × {Math.round(item.h)}</span></div>{item.type === 'sticky' && <div className="panel-row"><span>Note color</span><div className="color-row">{Object.keys(colors).map(color => <button key={color} className={`color-swatch ${item.color === color ? 'active' : ''}`} style={{ background: colors[color] }} onClick={() => onChange(item.id, { color })} aria-label={`${color} color`} />)}</div></div>}<button className="nav-item" style={{ padding: 0, marginTop: 8, color: 'var(--accent)' }} onClick={() => onChange(item.id, { locked: !item.locked })}>{item.locked ? 'Unlock object' : 'Lock object'}</button></> : <p style={{ color: 'var(--muted)', fontSize: 12, lineHeight: 1.5 }}>Select an item to edit its properties.</p>}<div className="panel-row"><span>Dock position</span><select className="menu-select" value={dockPosition} onChange={event => setDockPosition(event.target.value)}><option value="top">Top</option><option value="right">Right</option><option value="bottom">Bottom</option><option value="left">Left</option></select></div><div className="panel-row" style={{ marginTop: 8 }}><span>Grid</span><button className="nav-item" style={{ padding: '0 7px', minHeight: 25, background: grid ? 'var(--accent-soft)' : 'var(--line)' }} onClick={() => setGrid(value => !value)}>{grid ? 'On' : 'Off'}</button></div><div className="panel-row"><span>Theme</span><select value={theme} onChange={event => setTheme(event.target.value)}><option value="light">Light</option><option value="warm">Warm</option><option value="mint">Mint</option><option value="dark">Dark</option><option value="oled">OLED</option></select></div><div className="panel-row"><span>Accent</span><div className="color-row">{['#d86e50', '#577d6a', '#50739a', '#8a6b9f', '#c58a44'].map(color => <button key={color} className={`color-swatch ${accent === color ? 'active' : ''}`} style={{ background: color }} onClick={() => setAccent(color)} aria-label="Choose accent color" />)}</div></div></div>
}

function Properties({ item, accent, setAccent, theme, setTheme, grid, setGrid, dockPosition, setDockPosition, onClose, onChange }) {
  const accents = ['#d86e50', '#577d6a', '#50739a', '#8a6b9f', '#c58a44', '#d1495b', '#2a9d8f', '#e76f51', '#264653', '#6c757d']
  const updateNumber = (key, event) => onChange(item.id, { [key]: Number(event.target.value) || 0 })
  return <div className="floating-panel"><div className="panel-heading"><span><PanelRight size={14} /> Properties</span><button className="icon-button" title="Close properties" onClick={onClose}><X size={14} /></button></div>{item ? <><div className="panel-row"><span>Type</span><strong>{item.type}</strong></div><div className="panel-row"><span>Position</span><div className="property-pair"><input className="property-input" type="number" value={Math.round(item.x)} onChange={event => updateNumber('x', event)} aria-label="X position" /><input className="property-input" type="number" value={Math.round(item.y)} onChange={event => updateNumber('y', event)} aria-label="Y position" /></div></div><div className="panel-row"><span>Size</span><div className="property-pair"><input className="property-input" type="number" value={Math.round(item.w)} onChange={event => updateNumber('w', event)} aria-label="Width" /><input className="property-input" type="number" value={Math.round(item.h)} onChange={event => updateNumber('h', event)} aria-label="Height" /></div></div>{item.type === 'sticky' && <div className="panel-row"><span>Note color</span><div className="color-row">{Object.keys(colors).map(color => <button key={color} className={`color-swatch ${item.color === color ? 'active' : ''}`} style={{ background: colors[color] }} onClick={() => onChange(item.id, { color })} aria-label={`${color} color`} />)}</div></div>}{item.type === 'shape' && <div className="panel-row"><span>Fill</span><select className="menu-select" value={item.fill || 'solid'} onChange={event => onChange(item.id, { fill: event.target.value })}><option value="solid">Solid</option><option value="outline">Outline</option></select></div>}<button className="nav-item" style={{ padding: 0, marginTop: 8, color: 'var(--accent)' }} onClick={() => onChange(item.id, { locked: !item.locked })}>{item.locked ? 'Unlock object' : 'Lock object'}</button></> : <p style={{ color: 'var(--muted)', fontSize: 12, lineHeight: 1.5 }}>Select an item to edit its properties.</p>}<div className="panel-row" style={{ marginTop: 8 }}><span>Grid</span><button className="nav-item" style={{ padding: '0 7px', minHeight: 25, background: grid ? 'var(--accent-soft)' : 'var(--line)' }} onClick={() => setGrid(value => !value)}>{grid ? 'On' : 'Off'}</button></div><div className="panel-row"><span>Theme</span><select className="menu-select" value={theme} onChange={event => setTheme(event.target.value)}><option value="light">Light</option><option value="warm">Warm</option><option value="mint">Mint</option><option value="dark">Dark</option><option value="oled">OLED</option></select></div><div className="panel-row"><span>Accent</span><div className="color-row accent-colors">{accents.map(color => <button key={color} className={`color-swatch ${accent === color ? 'active' : ''}`} style={{ background: color }} onClick={() => setAccent(color)} aria-label="Choose accent color" />)}</div></div></div>
}

function UrlDialog({ onClose, onSubmit }) {
  const [url, setUrl] = useState('')
  return <div className="url-dialog" role="dialog" aria-label="Add image URL"><div className="panel-heading"><span><Link2 size={14} /> Add image URL</span><button className="icon-button" title="Close" onClick={onClose}><X size={14} /></button></div><input className="url-input" autoFocus value={url} placeholder="https://example.com/image.jpg" onChange={event => setUrl(event.target.value)} onKeyDown={event => { if (event.key === 'Enter') onSubmit(url) }} /><div className="dialog-actions"><button className="dialog-button secondary" onClick={onClose}>Cancel</button><button className="dialog-button primary" onClick={() => onSubmit(url)}>Add image</button></div></div>
}
